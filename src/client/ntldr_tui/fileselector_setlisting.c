...
void _ntFileSelectorSetListing(struct nt_file_selector_menu *m) {
    ...
    if (d) {
        ...
        while (i < m->listing->len) {
            const char *str = RSBGetAtIndex_cstr(m->listing, i);
            free((char *)str);
        ...
        if (m->page >= 1) {
            ...
            RSBAddElement_cstr(m->listing, prev_page);
        ...
        while (((dir = readdir(d)) != NULL) && i < pg_index_max) {
            ...
            if (!DirectoryExists(bf)) {
                folder_open = "";

                if (m->requirement == FSFile) {
                    for (size_t j = 0; j < m->wanted_fileformats->len; j++) {
                        const char *req = RSBGetAtIndex_cstr(m->wanted_fileformats, j);
            ...
            RSBAddElement_cstr(m->listing, (const char *)str_a);
            RSBAddElementColor(m->colors, c);
        ...
        if (m->listing->len == (m->items_per_page + has_prev)) {
            ...
            RSBAddElement_cstr(m->listing, next_page);
            RSBAddElementColor(m->colors, c);
        }

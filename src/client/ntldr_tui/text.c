...
void _ntTuiDrawTextCentered(const char *str, signed char x, signed char y, Color col) {
    ...
    if ((unsigned char)x == 0xFF) {
        ...
        int szX = __state.overlay_framebuffer.texture.width / _ntTuiEnvironment.base_font_size.x;
    ...
    if ((unsigned char)y == 0xFF) {
        ...
        int szY = __state.overlay_framebuffer.texture.height / _ntTuiEnvironment.base_font_size.y;
